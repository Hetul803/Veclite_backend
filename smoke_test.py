#!/usr/bin/env python3
"""
Smoke test script for Veclite backend API.
Tests that POST endpoints don't hang and respond quickly.

Usage:
    python smoke_test.py <API_URL> <API_KEY>

Example:
    python smoke_test.py https://memryxbackend-production.up.railway.app veclite_sk_...
"""
import sys
import time
import requests
...
def test_finalize_no_body(url: str, api_key: str) -> bool:
    """Test POST /finalize with NO BODY, only header (must return quickly, not hang)."""
    print("\nTesting POST /finalize (no body, header only)...")
    try:
        headers = {
            "X-API-Key": api_key,
...
def test_add_single_vector(url: str, api_key: str) -> bool:
    """Test POST /add with single vector + header (must return quickly)."""
    print("\nTesting POST /add (single vector, header only)...")
    try:
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python smoke_test.py <API_URL> <API_KEY>")
        print("Example: python smoke_test.py https://memryxbackend-production.up.railway.app veclite_sk_...")
        sys.exit(1)
    
    url = sys.argv[1].rstrip('/')
    api_key = sys.argv[2]
    
    print(f"Smoke Testing Veclite Backend")
    print(f"URL: {url}")
    ...
    # Test 2: POST /finalize with no body, header only
    results.append(("POST /finalize (no body)", test_finalize_no_body(url, api_key)))
    
    # Test 3: POST /add single vector with header
